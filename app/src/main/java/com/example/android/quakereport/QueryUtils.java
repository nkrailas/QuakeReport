...
public final class QueryUtils {
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            if (inputStream != null) {
                // Closing input stream could throw an IOException, and why makeHttpRequest (URL url)
                // method signature specifies that an IOException could be thrown
                inputStream.close();
            }
        }
        return jsonResponse;
    ...
    public static List<Earthquake> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            // Extract JSONArray associated with the key called "features", which represents
            // a list of feature (or earthquakes)
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            // Loop through each feature in the array
            for (int i = 0; i < earthquakeArray.length(); i++) {

                // Get a single earthquake at position i within list of earthquakes
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                // For a given earthquake, extract JSONObject associated with key “properties”
                // which represents a list of all properties for that earthquake
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                // Extract the values for the key called “mag," “place,” "time," "url"
                double magnitude = properties.getDouble("mag");
                String location = properties.getString("place");
                long time = properties.getLong("time");
                String url = properties.getString("url");

                // Create Earthquake object from magnitude, location, time, and url from JSON response
